...
// ac_shared_wr_mask_array_1D is a 1-dimensional memory that includes "ByteEnable" support.
...
// 
// Usage:
//   ac_shared_wr_mask_array<uint32, 0x1000, 8> mem;
...
class ac_shared_wr_mask_array_1D {
  public:
  ...
  void write(unsigned idx, T val, WriteMask mask_val=~0) {
      HLS_ASSERTION(idx < ac_shared_wr_mask_array_1D::dim1);
  ...
  struct elem_proxy {
    ...
    elem_proxy mask(ac_shared_wr_mask_array_1D::WriteMask _mask) {
...
class ac_shared_wr_mask_array_1D<T, Dim1, SliceWidth, true> {
  public:
  ...
  ac_shared_wr_mask_array_1D() {
#ifndef __SYNTHESIS__
   std::cout << "ac_shared_wr_mask_array_1D: Note: In Catapult tcl script need to set num_byte_enables to " << 
     num_slices << "\n";
#endif
  }
