...
public:
  ...
  sc_signal<bool> SC_NAMED(clk);

  msg_channel<unsigned>        SC_NAMED(out1);
  msg_channel<unsigned>        SC_NAMED(in1);

  sc_clock clock_alias;

  SC_CTOR(Top) :
    clock_alias("clock_alias", 2, SC_NS, 0.5, 0, SC_NS, true)
  {

    Connections::get_sim_clk().add_clock_alias(clock_alias.posedge_event(), clk.posedge_event());
